...
// Ranch drop-off system
if (place_meeting(x,y,objRanchGate))
{
	// SFX code here for cue that player is at gate
	
	// Option for player to drop off one monster at a time.
	if(key_drop)
	{
		// First slime gets dropped off first.
		if(global.slimeCatchCheck == 1)
		{
			// SFX code here for drop-off success.
			
			// Drops off monster; for now, just sets catch bool to false.
			global.slimeCatchCheck = 0;
			global.winCheck1 = 1;
		}
		// Second slime takes next priority.
		else if(global.staticSlimeCatchCheck == 1)
		{
			// SFX code here for drop-off success.
			
			// Drops off second monster.
			global.staticSlimeCatchCheck = 0;
			global.winCheck2 = 1;
		}
		// If no monsters collected, indicate player had nothing to drop off.
		else
		{
			// SFX code here for drop-off failure.
			// Optionally, global bool switch on to let HUD know
			// to tell player they had nothing to drop off.
		}
	}
	if(global.winCheck1 == 1 && global.winCheck2 == 1)
	{
